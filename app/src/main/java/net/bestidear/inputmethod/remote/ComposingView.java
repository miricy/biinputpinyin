...
public class ComposingView extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        float width;
        int height;
        height = mFmi.bottom - mFmi.top + this.getPaddingTop() + this.getPaddingBottom();

        if (null == mDecInfo) {
            width = 0;
        } else {
            width = this.getPaddingLeft() + this.getPaddingRight() + LEFT_RIGHT_MARGIN * 2;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float x, y;
        x = this.getPaddingLeft() + LEFT_RIGHT_MARGIN;
        y = -mFmi.top + this.getPaddingTop();

        mPaint.setColor(mStrColorHl);
        mHlDrawable.setBounds(this.getPaddingLeft(), this.getPaddingTop(), getWidth()
                - this.getPaddingRight(), getHeight() - this.getPaddingBottom());
    ...
    private void drawCursor(Canvas canvas, float x) {
        mCursor.setBounds((int) x, this.getPaddingTop(), (int) x
                + mCursor.getIntrinsicWidth(), getHeight() - this.getPaddingBottom());
        mCursor.draw(canvas);
    }
